...
// --- IN-MEMORY DATABASE ---
let tasks: Task[] = [
  { id: 1, name: 'Getriebewartung', durationHours: 4.5, requiredQualifications: [{ qualificationId: 1, requiredAmount: 1 }], requiredTools: [{ toolId: 1, requiredAmount: 1 }] },
  { id: 2, name: 'Rotorblatt Reparatur', durationHours: 8, requiredQualifications: [{ qualificationId: 2, requiredAmount: 2 }], requiredTools: [{ toolId: 2, requiredAmount: 1 }] }
];

let people: Employee[] = [
  { id: 1, firstname: 'Hannes', lastname: 'Fiedler', qualifications: [{ id: 1, name: 'Mechaniker', description: '' }] },
  { id: 2, firstname: 'Sören', lastname: 'Smit', qualifications: [{ id: 2, name: 'Kletterer', description: '' }] }
];
...
export const httpMockInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  if (url.endsWith('/qualifications')) {
    if (method === 'GET') return of(new HttpResponse({ status: 200, body: qualifications })).pipe(delay(100));
    if (method === 'POST') {
  ...
  if (url.endsWith('/persons')) {
    if (method === 'GET') {
      const summaries: EmployeeSummary[] = people.map(p => ({ id: p.id, firstname: p.firstname, lastname: p.lastname }));
  ...
  if (url.match(/\/persons\/\d+$/)) {
    const id = getId();
    if (method === 'GET') return of(new HttpResponse({ status: 200, body: people.find(p => p.id === id) }));
    if (method === 'PATCH') {
      const b = body as any;
      people = people.map(p => p.id === id ? {
        ...p,
        ...b,
        qualifications: b.qualificationIds ? qualifications.filter(q => b.qualificationIds.includes(q.id)) : p.qualifications
      } : p);
  ...
  if (url.endsWith('/tasks')) {
    if (method === 'GET') {
      const summaries: TaskSummary[] = tasks.map(t => ({ id: t.id, name: t.name, durationHours: t.durationHours }));
      return of(new HttpResponse({ status: 200, body: summaries }));
    }
    if (method === 'POST') {
  ...
  if (url.match(/\/tasks\/\d+$/)) {
    const id = getId();
    if (method === 'GET') return of(new HttpResponse({ status: 200, body: tasks.find(t => t.id === id) }));
    if (method === 'PATCH') {
